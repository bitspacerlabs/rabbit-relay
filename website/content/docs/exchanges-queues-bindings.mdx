---
title: Exchanges, Queues, and Bindings
description: How RabbitMQ routes messages through exchanges, bindings, and queues
icon: Share2
---
...
For publisher-only code, use `broker.exchange()` to declare just the exchange — no
queue, no binding, no consumer. This keeps topology ownership clean in multi-process
setups: the producer owns the exchange; the consumer owns the queue and bindings.
...
The important part for publishing is the exchange. A publisher interface has an empty
queue and binding plan:
...
> Only use `.queue(...).exchange(...)` when the same process also consumes from the
> queue via `consume()`.
...
- Use `broker.exchange(...)` for exchange-only (publisher-only) topology
