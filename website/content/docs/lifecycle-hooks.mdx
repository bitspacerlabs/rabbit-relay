...
description: Expose broker operational events for logging, metrics, and production visibility
icon: Clock
---
...
<Callout title="Note">
  If a lifecycle hook throws, the error is caught and logged.
  Rabbit Relay continues running and the broker operation is not blocked.
...
<Cards>
  <Card title="Graceful Shutdown" description="Clean broker teardown and handler drain" href="/docs/graceful-shutdown" />
  <Card title="Health Checks" description="Monitor broker and consumer health" href="/docs/health-checks" />
</Cards>
