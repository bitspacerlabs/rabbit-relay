...
description: Read-only topology plan that returns what Rabbit Relay knows about without contacting RabbitMQ.
icon: ListTree
---
...
For passive checks against RabbitMQ, use [Topology Validation](/docs/topology-validation).

For topology ownership behavior, use [Topology Modes](/docs/topology-modes).
...
<Cards>
  <Card title="Topology CLI" description="CLI commands for plan, validate, diff, and DLQ inspection" href="/docs/cli-reference" />
  <Card title="Topology Validation" description="Passive checks that planned topology exists" href="/docs/topology-validation" />
</Cards>
