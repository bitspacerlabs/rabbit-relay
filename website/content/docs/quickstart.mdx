...
<Callout title="Use RPC deliberately" type="warn">
  RPC creates tighter service coupling than events. Prefer events when the
  workflow does not need an immediate reply.
</Callout>
...
Use `traceFrom()` when creating a child event from an existing event. It
preserves the correlation ID and sets the causation ID to the parent event's ID.
...
<Tabs items={["lifecycle.ts", "topology.ts", "redrive.ts"]}>
  <Tab value="lifecycle.ts">
