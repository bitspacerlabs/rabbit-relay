---
title: Middleware
description: Add local behavior around consumer handlers on a single broker interface
icon: Workflow
---
...
<Callout title="Note">
  The middleware API intentionally avoids exposing raw RabbitMQ primitives
  (message, channel, manual ack/nack) to prevent double-ack bugs. Advanced
  middleware can be added later if needed.
</Callout>
...
<Cards>
  <Card title="Plugins" description="Process-wide cross-cutting behavior" href="/docs/plugins" />
  <Card title="Lifecycle Hooks" description="Operational events for logging and metrics" href="/docs/lifecycle-hooks" />
</Cards>
